...
for (root, _, fns) in os.walk(pl.Path("NormalizedFace").joinpath("ClientNormalized")):
	for fn in fns:
		im = cv2.imread(str(pl.Path(root).joinpath(fn)))

		if im is None:
			print(root + os.path.sep + fn)

		res = np.vstack((res, np.append(lv.feature_extractor(im), np.array([1], dtype=np.uint32))))

for (root, _, fns) in os.walk(pl.Path("NormalizedFace").joinpath("ImposterNormalized")):
	for fn in fns:
		im = cv2.imread(str(pl.Path(root).joinpath(fn)))
